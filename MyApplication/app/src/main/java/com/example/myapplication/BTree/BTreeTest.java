...
public class BTreeTest {
    ...
    @Test(timeout = 1000)
    public void singleSplitAtRootOddOrderTest() {
    ...
    @Test(timeout = 1000)
    public void singleSplitAtRootEvenOrderTest() {
        // Insert a few with order 4
        ...
        // Split should have been on index 1 of the keys list within the root (left split preference like as stated in readme.md)
        String expected = "[2]\n├─[1]\n├─[3, 4]";
        assertEquals(
                "\nSimple insertion failed with order 4.\nYour tree should look like:\n" + expected + "\nBut it actually looks like:\n" + bTree.display(),
                "{order=4, root={keys=[2], children=[{keys=[1], children=[]}, {keys=[3, 4], children=[]}]}}",
                bTree.toString()
        );
    }
    ...
    @Test(timeout = 1000)
    public void splitMiddleChildTest() {
    ...
    public void manyInsertTest() {
        ...
        expected = "[4, 8]\n├─[2]\n\t├─[1]\n\t├─[3]\n├─[6]\n\t├─[5]\n\t├─[7]\n├─[11, 14]\n\t├─[9, 10]\n\t├─[12, 13]\n\t├─[15, 16]";
        assertEquals(
                "\nAdvanced insertion failed with order 4.\nYour tree should look like:\n" + expected + "\nBut it actually looks like:\n" + bTree.display(),
                "{order=4, root={keys=[4, 8], children=[{keys=[2], children=[{keys=[1], children=[]}, {keys=[3], children=[]}]}, {keys=[6], children=[{keys=[5], children=[]}, {keys=[7], children=[]}]}, {keys=[11, 14], children=[{keys=[9, 10], children=[]}, {keys=[12, 13], children=[]}, {keys=[15, 16], children=[]}]}]}}",
                bTree.toString()
        );

        // Insert a lot with order 6
        bTree = new BTree<>(6);
        bTree.insert(1);
        bTree.insert(2);
        bTree.insert(3);
        ...
        expected = "[3, 6, 9, 13]\n├─[1, 2]\n├─[4, 5]\n├─[7, 8]\n├─[10, 11, 12]\n├─[14, 15, 16]";
        assertEquals(
                "\nAdvanced insertion failed with order 6.\nYour tree should look like:\n" + expected + "\nBut it actually looks like:\n" + bTree.display(),
                "{order=6, root={keys=[3, 6, 9, 13], children=[{keys=[1, 2], children=[]}, {keys=[4, 5], children=[]}, {keys=[7, 8], children=[]}, {keys=[10, 11, 12], children=[]}, {keys=[14, 15, 16], children=[]}]}}",
                bTree.toString()
        );
    }
